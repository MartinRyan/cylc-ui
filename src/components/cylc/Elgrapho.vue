...
export default {
  ...
  metaInfo () {
    return {
      title: this.getPageTitle('App.elgrapho', { name: this.workflowId })
    }
  },

  beforeDestroy () {
    elgraphoService.unregister(this)
  },
  ...
  methods: {
    subscribe (queryName) {
      const id = elgraphoService.subscribe(
        this,
    ...
    unsubscribe (queryName) {
      if (queryName in this.subscriptions) {
        elgraphoService.unsubscribe(this.subscriptions[queryName].id)
      }
    },
    ...
    async workflowUpdated (workflows) {
      ...
      try {
        ...
        if (!isEmpty(elements)) {
          ...
          console.debug('elements not empty <><><><> ', elements)
        }
      } catch (error) {
    ...
    async updateGraph (elements) {
      try {
        ...
        if (!isEmpty(elements)) {
          ...
          console.debug('passed elements DATA ===> ', elements)
          /* eslint new-cap: [0] */
          let model = {
            nodes: [],
            edges: []
